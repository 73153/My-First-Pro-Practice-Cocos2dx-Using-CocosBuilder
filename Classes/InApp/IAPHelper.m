...
//        NSSet *productIdentifiers = [NSSet setWithObjects:kInAppProductIdentifierEffileTower,nil];
...
- (void)provideContent:(NSString *)productIdentifier  {
    ...
    NSLog(@"Toggled flag for: %@ and count->%lu",productIdentifier,(unsigned long)_purchasedProducts.count);
...
- (void)paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue {
    ...
    if (queue.transactions.count) {
        ...
        if ([self.delegate respondsToSelector:@selector(restoreCompletedWithTransactions:)])
            [self.delegate restoreCompletedWithTransactions:queue.transactions];
    }
    else {
        if ([self.delegate respondsToSelector:@selector(noProductToRestore)])
            [self.delegate noProductToRestore];
    }
}
- (void)paymentQueue:(SKPaymentQueue *)queue restoreCompletedTransactionsFailedWithError:(NSError *)error {
    if ([self.delegate respondsToSelector:@selector(failedProductPurchased:)])
        [self.delegate failedProductPurchased:nil];
}
@end
