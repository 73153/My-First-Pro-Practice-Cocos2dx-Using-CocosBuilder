...
#import "../ObjCCalls/HelloWorldScene.h"

@implementation MailSender

-(void) sendMailUsingExternalApp:(const char *)score
{
    NSString *message = [NSString stringWithFormat:@"<p>Send a friend request to get %s coins!</p>",score];
    [self sendEmailTo:@""
               withCC:@""
              withBCC:@""
...
- (void)sendMailUsingInAppMailer:(const char *)score
{
    if ([MFMailComposeViewController canSendMail])
    {
        ...
        NSString *emailBody = [NSString stringWithFormat:@"<p>Send a friend request to get %s coins!</p>",score];
